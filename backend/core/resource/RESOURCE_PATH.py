...
# Replacing gsuid_core.data_store.get_res_path with fixed data directory
from pathlib import Path as _Path

def _get_data_path() -> _Path:
    """Return base data directory. Override via environment variable."""
    import os
    env_path = os.environ.get("WUWA_DATA_PATH", "")
    if env_path:
        return _Path(env_path)
    # Default: backend/data/
    return _Path(__file__).resolve().parent.parent.parent / "data"

MAIN_PATH = _get_data_path() / "XutheringWavesUID"
...
def init_dir():
    """Create only directories actually used by the web version."""
    for i in [
        MAIN_PATH,
        PLAYER_PATH,
        GACHA_BACKUP_PATH,
...
# 设置 Jinja2 环境 - templates now live under backend/login_pages/templates/
_BACKEND_ROOT = MAIN_PATH.parent.parent  # backend/
TEMP_PATH = _BACKEND_ROOT / "login_pages" / "templates"
...
# Custom template path (optional, may not exist)
CUSTOM_TEMP_PATH = MAIN_PATH / "show"
...
) if CUSTOM_TEMP_PATH.exists() else waves_templates

# Captcha directory (not used in web version)
CAPTCHA_PATH = Path(__file__).parent.parent / "api" / "captcha"
