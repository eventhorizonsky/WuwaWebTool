...
from typing import Any, Dict, List, TypeVar, Callable, Optional, Coroutine, overload, Union
...
# gsuid_core.subscribe not needed in web version - stub out send_master_info
...
@overload
def async_func_lock(*, keys: Union[List[str], None] = None) -> Callable[[F], F]: ...


@overload
def async_func_lock(_func: F, *, keys: Union[List[str], None] = None) -> F: ...


# 异步函数参数锁
def async_func_lock(
    _func: Union[F, None] = None,
    *,
...
# Web version: no bot master to notify; stub out send_master_info
async def _send_master_info_impl(msg: str):
    """Web version: just log important messages."""
    for i in filter_msg:
        if i in msg:
            return
